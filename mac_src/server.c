...
void init_server()
{
    ...
    //EVFILT_READ数据可读
    EV_SET(&event_change,server_socket,EVFILT_READ,EV_ADD,0,0,NULL);
    //int kevent(int kq, const struct kevent *changelist, int nchanges, struct kevent *eventlist, int nevents, const struct timespec *timeout);
    //timeout为NULL，kevent将阻塞，直至有事件发生为止。
    kevent(kq,&event_change,1,NULL,0,NULL);

    while(1)
    {
        int ret=kevent(kq,NULL,0,events,CONCURRENT_MAX+2,NULL);
        if(ret<0)
        {
            printf("Kevent Error !\n");
            continue;
        ...
        {
            struct kevent current_event=events[index];
